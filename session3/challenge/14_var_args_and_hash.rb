...
def problem_14(*n)
  problem = n.pop[:problem] if n.last.is_a? Hash
  problem ||= :count_clumps
  return count_clumps(*n) if problem == :count_clumps
  return same_ends(*n) if problem == :same_ends
...
def same_ends(n, *array)
  array.slice(0, n) == array.slice(- n, n)
end
